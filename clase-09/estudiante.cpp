...
bool Controlador::aprueba(){
    return this->promedio() >= 7;
}

int Controlador::ayudar()
{
    if (this->modelo->getNota1() <= this->modelo->getNota2() && this->modelo->getNota1() <= this->modelo->getNota3())
    {
        this->actualizarNota(1, this->modelo->getNota1() + 1);
        return 1;
    }
    else if (this->modelo->getNota2() <= this->modelo->getNota1() && this->modelo->getNota2() <= this->modelo->getNota3())
    {
        this->actualizarNota(2, this->modelo->getNota2() + 1);
        return 2;
    }
    else
    {
        this->actualizarNota(3, this->modelo->getNota3() + 1);
        return 3;
    }
    return -1;
}
...
void Vista::estaAprobado(){
    if (this->control->aprueba()){
        cout << "\nEl estudiante aprueba" << endl;
    }else{
        cout << "\nEl estudiante reprueba" << endl;
    }
}

void Vista::ayudar(){
    int nota = this->control->ayudar();
    if (nota >0){
        cout << "\nSe ha ayudado con un punto a la nota " << nota << endl;
    }
}
